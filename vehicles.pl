...
vehiculo_premium(Info) :-
    findall(
        [Precio, Marca, Ref, Tipo, Anno],
        vehiculo(Marca, Ref, Tipo, Precio, Anno),
        Todos
    ),
    sort(1, @>=, Todos, [[P, M, R, T, A]|_]),
    Info = premium(marca(M), modelo(R), tipo(T), precio(P), anno(A)).
...
mejor_oferta(Info) :-
    ...
    sort(1, @=<, Todos, [[P, M, R, T, A]|_]),
    Info = oferta(marca(M), modelo(R), tipo(T), precio(P), anno(A)).
